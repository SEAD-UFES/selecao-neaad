...
<html>
    <head>     
        <title>Seleção EAD - Manter participação em evento</title>
        ...
        $edicao = isset($_GET['idPartEvento']);
        if ($edicao) {
    ...
    <body>  
        ...
        <div id="main">
            <div id="container" class="clearfix">
                ...
                <div id="breadcrumb">
                    <h1 class="narrow">Você está em: Candidato > <a href="listarPartEvento.php">Currículo</a> > <strong><?php if (!$edicao) { ?>Adicionar Participação em Evento<?php } else { ?>Editar Participação em Evento<?php } ?></strong></h1>
                </div>

                <?php print Util::$MSG_CAMPO_OBRIG_TODOS; ?>

                <div class="col-full m02">
                    <div id="divErCriarPartEvento" style="display: none" class="alert alert-danger">
                        Você já cadastrou uma participação em evento com os parâmetros informados. Por favor, atualize o item existente.
                    </div>

                    <?php if (!$edicao) { ?>
                        <form id="formCadastro" class="form-horizontal" method="post" action='<?php print "$CFG->rwww/controle/CTCurriculo.php?acao=criarPartEvento" ?>'>
                        <?php } else { ?>
                            <form id="formEdicao" class="form-horizontal" method="post" action='<?php print "$CFG->rwww/controle/CTCurriculo.php?acao=editarPartEvento" ?>'>
                            ...
                            <div class="form-group">                                
                                ...
                                <div class="col-xs-12 col-sm-8 col-md-8">
                                    <span><?php
                                        if (!$edicao) {
                                            impressaoTipoPartEvento();
                                        } else {
                                            impressaoTipoPartEvento($partEvento->getPEV_TP_ITEM(), $edicao);
                                        }
                                        ?></span>
                                </div> 
                            </div>
                                ...
                                <div class="col-xs-12 col-sm-8 col-md-8">
                                    <span><?php
                                        if (!$edicao) {
                                            ...
                                            impressaoArea($partEvento->getPEV_ID_AREA_CONH(), "idAreaConh", $edicao);
                                        }
                                        ?></span>
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-xs-12 col-sm-8 col-md-8">
                                    <input class="form-control" type="text" id="qtde" name="qtde" size="5" maxlength="5" value="<?php $edicao ? print $partEvento->getPEV_QT_ITEM() : print ""; ?>">
                                </div>
                            </div>
    ...
    <script type="text/javascript">
        $(document).ready(function () {
            ...
            {
                return tipo == '<?php print ParticipacaoEvento::getTpFixarArea(); ?>';
            }

            function getIdSubarea() {
                var id = isFixarArea($("#tpPartEvento").val()) ? "'<?php print ParticipacaoEvento::getSubAreaFixa(); ?>'" : "";

                if (id === "")
                {
                    id = <?php $edicao ? print "\"'{$partEvento->getPEV_ID_SUBAREA_CONH()}'\"" : print "''" ?>;
                }
                return id;
            }

            function getIdArea() {
                var id = isFixarArea($("#tpPartEvento").val()) ? '<?php print ParticipacaoEvento::getAreaFixa(); ?>' : "";

                if (id === "")
                {
                    id = <?php $edicao ? print "'{$partEvento->getPEV_ID_AREA_CONH()}'" : print "''" ?>;
                }
                return id;
            }
            ...
            var funcaoFixaArea = function ()
            {
                ...
                if (isFixarArea($("#tpPartEvento").val())) {
            ...
            $("#tpPartEvento").change(funcaoFixaArea);
            ...
            $("#formCadastro").validate({
                submitHandler: function (form) {
                    ...
                    $("#divErCriarPartEvento").hide();
                    $.ajax({
                        type: "POST",
                        url: getURLServidor() + "/controle/CTAjax.php?val=partEvento",
                        data: {"tpPartEvento": $("#tpPartEvento").val(), "idAreaConh": $("#idAreaConh").val(), "idSubareaConh": $("#idSubareaConh").val(), "idUsuario": getIdUsuarioLogado()},
                        dataType: "json",
